...
module.exports = {
  ...
  communityPosts: {
    ...
    async toggleReaction(postId, userId) {
      ...
      const existing = await prisma.community_reactions.findFirst({
        where: {
          post_id: postId,
          user_id: userId,
          reaction_type: "Like",
      ...
      if (existing) {
        // Unlike — remove the reaction and decrement count
        await prisma.community_reactions.delete({ where: { id: existing.id } });
        await prisma.community_posts.update({
          where: { id: postId },
          data: { reactions_count: { decrement: 1 } },
        });
        return { liked: false, postId };
      } else {
        // Like — create the reaction and increment count
        await prisma.community_reactions.create({
          data: {
            post_id: postId,
            user_id: userId,
            reaction_type: "Like",
        ...
        await prisma.community_posts.update({
          where: { id: postId },
          data: { reactions_count: { increment: 1 } },
        });
    ...
    async addComment(postId, authorId, content) {
      ...
      return {
        ...
        author: {
          ...
          role: comment.users.role?.name || "Student",
        },
      };
    },
    ...
    async getComments(postId) {
      ...
      return comments.map((c) => ({
        ...
        author: {
          id: c.users.id,
          name: c.users.name,
          role: c.users.role?.name || "Student",
        },
      }));
    },
  ...
  communityPosts: {
    ...
    _include: (userId) => ({
      ...
      community_post_tags: {
        include: { community_tags: { select: { id: true, name: true, slug: true } } },
      },
      community_reactions: userId
        ? { where: { user_id: userId }, select: { id: true, reaction_type: true } }
        : false,
      community_bookmarks: userId
    ...
    _map: (p, userId) =>
      p && {
        ...
        author: {
          id: p.users.id,
          name: p.users.name,
          role: p.users.role?.name || 'Unknown',
          avatar: p.users.name
            .split(' ')
            .map((n) => n[0])
            .join('')
        ...
        tags: (p.community_post_tags || []).map((t) => ({
          id: t.community_tags.id,
          name: t.community_tags.name,
          slug: t.community_tags.slug,
        })),
        liked: userId ? (p.community_reactions || []).length > 0 : false,
        bookmarked: userId ? (p.community_bookmarks || []).length > 0 : false,
      },

    // ── CRUD ────────────────────────────────────────────────────────
    getFeed: async ({ post_type, page = 1, limit = 20, userId, userRole } = {}) => {
      const roleMap = {
        'student': 'Students',
        'educator': 'Educators',
        'employer': 'Employers',
      ...
      const mappedRole = roleMap[userRole?.toLowerCase()] || 'Public';

      const visibilityFilter = {
        OR: [
          { visibility: { has: 'Public' } },
          { visibility: { has: mappedRole } },
      ...
      const posts = await prisma.community_posts.findMany({
        ...
        take: limit,
        include: {
          users: { select: { id: true, name: true, role: { select: { name: true } } } },
          community_post_tags: {
            include: { community_tags: { select: { id: true, name: true, slug: true } } },
          },
          ...(userId
            ? {
                community_reactions: { where: { user_id: userId }, select: { id: true, reaction_type: true } },
                community_bookmarks: { where: { user_id: userId }, select: { id: true } },
              }
            : {}),
        },
      });
      const total = await prisma.community_posts.count({ where });
      return {
        posts: posts.map((p) => {
          const self = { communityPosts: { _map: null } };
          return {
            id: p.id,
            author_id: p.author_id,
            title: p.title,
            ...
            author: {
              ...
              name: p.users.name,
              role: p.users.role?.name || 'Unknown',
              avatar: p.users.name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2),
            },
            tags: (p.community_post_tags || []).map((t) => ({
              id: t.community_tags.id,
              name: t.community_tags.name,
    ...
    findById: async (id, userId) => {
      const p = await prisma.community_posts.findUnique({
        where: { id },
        include: {
          users: { select: { id: true, name: true, role: { select: { name: true } } } },
          community_post_tags: {
    ...
    create: async ({ author_id, title, content, post_type, visibility = ['Public'], media_url = null, metadata = null, tags = [] }) => {
      // Upsert tags and link them
      const tagRecords = await Promise.all(
        tags.map((name) => {
          const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
          return prisma.community_tags.upsert({
            where: { slug },
            update: { usage_count: { increment: 1 } },
            create: { name, slug, usage_count: 1 },
          });
        })
      );
      ...
      return {
        ...
        author: {
          id: post.users.id,
          name: post.users.name,
          role: post.users.role?.name || 'Unknown',
          avatar: post.users.name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2),
        },
        tags: (post.community_post_tags || []).map((t) => ({
          id: t.community_tags.id,
          name: t.community_tags.name,
          slug: t.community_tags.slug,
        })),
    ...
    update: async (id, { title, content, post_type, visibility, status, media_url, metadata }) => {
      const patch = {};
      ...
      return safeQuery(
        prisma.community_posts.update({ where: { id }, data: patch })
      );
    },
    ...
    // ── Reactions ───────────────────────────────────────────────────
    toggleReaction: async (userId, postId, reactionType = 'Like') => {
      const existing = await prisma.community_reactions.findFirst({
      ...
      if (existing) {
        ...
        await prisma.community_reactions.create({
          data: { user_id: userId, post_id: postId, reaction_type: reactionType },
        });
        await prisma.community_posts.update({
          where: { id: postId },
          data: { reactions_count: { increment: 1 } },
    ...
    toggleBookmark: async (userId, postId) => {
      const existing = await prisma.community_bookmarks.findUnique({
        where: { user_id_post_id: { user_id: userId, post_id: postId } },
      });
      if (existing) {
        await prisma.community_bookmarks.delete({ where: { id: existing.id } });
    ...
    getBookmarks: async (userId) => {
      const rows = await prisma.community_bookmarks.findMany({
        where: { user_id: userId },
        orderBy: { created_at: 'desc' },
        include: {
          community_posts: {
      ...
      return rows
        .filter((r) => !r.community_posts.deleted_at)
        .map((r) => {
    ...
    addComment: async (postId, authorId, content) => {
      ...
      return {
        ...
        author: {
          id: comment.users.id,
          name: comment.users.name,
          role: comment.users.role?.name || 'Unknown',
          avatar: comment.users.name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2),
        },
      };
    },
    ...
    getComments: async (postId) => {
      ...
      return comments.map((c) => ({
        ...
        author: {
          id: c.users.id,
          name: c.users.name,
          role: c.users.role?.name || 'Unknown',
          avatar: c.users.name.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2),
        },
      }));
    },
  },
