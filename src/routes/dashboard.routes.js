...
router.get("/student", authRequired, async (req, res, next) => {
  try { res.json(await repo.studentDashboard(req.user.sub)); }
  catch (err) { next(err); }
});
 ...
 */
router.get("/educator", authRequired, permissionRequired("dashboards:educator"), async (req, res, next) => {
  try { res.json(await repo.educatorInsights(req.user.sub)); }
  catch (err) { next(err); }
});
 ...
 */
router.get("/employer", authRequired, permissionRequired("dashboards:employer"), async (req, res, next) => {
  try { res.json(await repo.employerInsights(req.user.sub)); }
  catch (err) { next(err); }
});
