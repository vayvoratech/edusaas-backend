...
router.get(
  ...
  async (req, res, next) => {
    try {
      const user = await repo.users.findById(
        req.user.sub
      );
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      let isAuthorizedEducator =
        req.user.role === "educator" &&
        targetUser?.role === "student";

      let isAuthorizedEmployer = false;

      // Existing employer authorization logic
      if (
       ...
       * 4. Employer can view students who applied
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const user =
        await repo.users.findById(requestedId);
